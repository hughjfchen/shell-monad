...
data Expr
  ...
  | -- | a comment
    Comment L.Text
  | -- | expressions run in a sub-shell
    Subshell L.Text [Expr]
  | -- | expressions run in a group
    Group L.Text [Expr]
  | -- | Piping the first Expr to the second Expr
    Pipe Expr Expr
  | -- | &&
    And Expr Expr
  | -- | ||
    Or Expr Expr
  | -- | Redirects a file handle of the Expr
...
data RedirSpec
  = -- | redirect the fd to a file
    RedirToFile Fd FilePath
  | -- | append to file
    RedirToFileAppend Fd FilePath
  | -- | use a file as input
    RedirFromFile Fd FilePath
  | -- | redirect first fd to the second
    RedirOutput Fd Fd
  | -- | same, but for input fd
    RedirInput Fd Fd
  | -- | use a here document as input
    RedirHereDoc L.Text
    ...
    go (EnvWrap i n localenvs e) =
      let (lp, sep) =
       ...
       in lp
            ...
            <> "() { : "
            <> sep
            <> L.intercalate sep (map (go . indent) e)
            <> sep
            <> lp
            <> "}"
            <> sep
            <> lp
            <> fmtlocalenvs localenvs
            <> " "
            <> n
...
eofMarker t = go (1 :: Integer)
  where
    go n =
      let marker = "EOF" <> if n == 1 then "" else L.pack (show n)
...
withEnv n v (Script f) = Script $ addEnv . f
  where
    ...
    -- environment.
    addEnv (e, env, _) =
      let localenv = (n, toTextParam v env)
       in case e of
